...
func SetNewHeaderTrain(timeStamp uint64) []byte {
	SetNewBodyTrain()
...
func (header *Block_Header_Train) VerifyTrainHeaderHash() bool {
	copyHeader := &Block_Header_Train{header.Height, header.PreviousHash,
...
func (body *Block_Body_Train) VerifyTrainBodyHash() bool {
	copyBody := &Block_Body_Train{nil, body.InfoHashes}
...
func GetInfoTrainHashes() [][]byte {
	var hashes [][]byte
	for _, infoTrain := range common.InfoTrainPool {
		hashes = append(hashes, infoTrain.Hash)
	}
	return hashes
}
...
func CreateGenesisBlock() (*Block_Header_Train, *Block_Body_Train) {
	...
	//info.Sign()
	//info.Sign()
	common.InfoTrainPool = append(common.InfoTrainPool, info)
