...
func NewTCPAgent(consensus *bdls.Consensus, privateKey *ecdsa.PrivateKey) *TCPAgent {
	agent := new(TCPAgent)
	consensus.AddSendToSuperior(sendToSuperior)
	consensus.AddSendToSubordinates(sendToSubordinates)
...
func (agent *TCPAgent) Update() {
	...
	default:
		...
		timer.SystemTimedSched.Put(agent.Update, time.Now().Add(global.UpdateDelay*time.Millisecond))
	}
}
func (agent *TCPAgent) InstantUpdate() {
	agent.Lock()
	defer agent.Unlock()
	select {
	case <-agent.die:
...
func (tagent *TCPAgent) ConnectPeer() {
	var peers []string
	for _, pair := range global.Cfg.Group {
		peers = append(peers, "localhost:5"+pair[1])
	}
		...
		go func(raddr string) {
			for {
				...
				if err == nil {
					...
					//fmt.Println("global.PortId:", global.PortId)
					g := Gossip{Command: CommandType_AddPeer, Message: []byte{}}
					// proto marshal
					out, err := proto.Marshal(&g)
					if err != nil {
						panic(err)
...
func (tagent *TCPAgent) handleMsg(conn net.Conn) {
	msgLength := make([]byte, MessageLength)
	...
	switch gossip.Command {
	case CommandType_AddPeer:
		fmt.Println("add peer:", conn.RemoteAddr())
		// peer endpoint created
		p := NewTCPPeer(conn, tagent)
		tagent.AddPeer(p)
		p.InitiatePublicKeyAuthentication()
	case CommandType_RequestConsensus:
		if global.StatusLogin == global.BureauStatus {
			//tagent.Propose(gossip.Message)
			global.RWMutexBlock.Lock()
			global.Blocks.Blocks = append(global.Blocks.Blocks, gossip.Message)
			global.RWMutexBlock.Unlock()
			fmt.Println("收到下级发来的待共识区块，即将开启共识...")
		}
		defer conn.Close()
	case CommandType_AgreeConsensus:
		fmt.Println("收到上级发来的审批 区块即将入链...:")
		//包含多个区块信息
		go bdls.UpdateBlockchain(gossip.Message)
		defer conn.Close()
	}

}
func (tagent *TCPAgent) ReceiverTcp() {
	// start listener
	tcpaddr, err := net.ResolveTCPAddr("tcp", ":"+global.PortId)
	if err != nil {
		return
	}
	...
	for {
		...
		go tagent.handleMsg(conn)
	}
}
func (tagent *TCPAgent) HttpServer() {
	// 1.创建路由
	r := gin.Default()
	// 2.绑定路由规则，执行的函数
	...
	r.POST("/query", func(c *gin.Context) {
		...
		global.MyError(err)
		json.Unmarshal(bts, &dict)
		fmt.Println("收到查询请求，查询类型：", dict["type"], "；待查询账户：", dict["account"])
		pub, err := hex.DecodeString(dict["account"])
		global.MyError(err)
		if dict["type"] == global.BlockchainTrain {
			//查询列车链
			train.QueryTrainInfo(pub)
		} else {
			//查询用户链
			user.QueryUserInfo(pub)
		}
	})
	r.POST("/register", func(c *gin.Context) {
		...
		bts, err := c.GetRawData()
		global.MyError(err)
		err = json.Unmarshal(bts, &dict)
		global.MyError(err)
		number, err := strconv.Atoi(dict["number"])
		global.MyError(err)
		infos := make([]*global.ConsensusInfo, number)
		var previousUser []byte
		if user.PreviousUser == nil {
			previousUser = global.GetPrevious(global.PublicKey)
		} else {
		...
		for i := 0; i < number; i++ {
			...
			key := privateKey.D
			fmt.Println("账户的D值为：")
			account := global.GetFillBytes(privateKey.X, privateKey.Y)
			global.MapUserData[hex.EncodeToString(account)] = key
			infoUser := &common.InfoUser{nil, global.PublicKey, nil, account, []byte("attendant"), previousUser}
			infoUser.Sign(*global.PrivateKey)
			previousUser = infoUser.Hash
			consensusInfo := &global.ConsensusInfo{global.BlockchainUser, infoUser.SerializeInfoUser()}
			infos[i] = consensusInfo
		}
		user.PreviousUser = previousUser
		global.RWMutexConsensusPool.Lock()
		global.ConsensusInfoPool.Infos = append(global.ConsensusInfoPool.Infos, infos...)
		global.RWMutexConsensusPool.Unlock()
		if dict["status"] == "bureau" && global.StatusLogin == global.BureauStatus {
			fmt.Println("收到用户注册请求，即将注册列车层账户数量：", number)
			block := global.SerializeBlockData()
			global.RWMutexBlock.Lock()
			global.Blocks.Blocks = append(global.Blocks.Blocks, block)
			global.RWMutexBlock.Unlock()
		} else {
			fmt.Println("收到用户注册请求，即将注册乘务员账户数量：", number)
		}
		return
	})
	r.POST("/report", func(c *gin.Context) {
		...
		err = json.Unmarshal(bts, &dict)
		global.MyError(err)
		fmt.Println("收到来自乘务员账户上报的高铁数据...")
		if len(dict["account"]) < 10 {
			fmt.Println("账户错误，请重试...")
			return
		}
		d := new(big.Int)
		d, _ = d.SetString(dict["account"], 10)
		fmt.Println("该乘务员账户D值为：", d)
		priv := new(ecdsa.PrivateKey)
		priv.PublicKey.Curve = bdls.S256Curve
		priv.D = d
		priv.PublicKey.X, priv.PublicKey.Y = bdls.S256Curve.ScalarBaseMult(priv.D.Bytes())
		pubKey := global.GetFillBytes(priv.X, priv.Y)
		//获取PreviousTrain
		var previousTrain []byte
		if _, ok := train.MapCurrentTrain[hex.EncodeToString(pubKey)]; !ok {
			previousTrain = global.GetPrevious(pubKey)
		} else {
			previousTrain = train.MapCurrentTrain[hex.EncodeToString(pubKey)]
		}
		//当前信息为train，加入ConsensusInfoPool
		infoTrain := &common.InfoTrain{nil, pubKey, nil, dict["event"], dict["name"], time.Now().String(), dict["info"], previousTrain}
		infoTrain.Sign(*priv)
		train.MapCurrentTrain[hex.EncodeToString(pubKey)] = infoTrain.Hash
		consensusInfo := &global.ConsensusInfo{global.BlockchainTrain, infoTrain.SerializeInfoTrain()}
		global.RWMutexConsensusPool.Lock()
		global.ConsensusInfoPool.Infos = append(global.ConsensusInfoPool.Infos, consensusInfo)
		fmt.Println("已经加入共识信息池...")
		reqCount++
		global.RWMutexConsensusPool.Unlock()
		global.RWMutexConsensusPool.RLock()
		lengthPool := len(global.ConsensusInfoPool.Infos)
		global.RWMutexConsensusPool.RUnlock()
		if lengthPool > global.PoolFill {
			tagent.InstantUpdate()
		}
	...
	r.POST("/login", func(c *gin.Context) {
		...
		global.MyError(err)
		json.Unmarshal(bts, &dict)
		fmt.Println("收到登录请求，待验证账户：", dict["account"], "即将开始查询...")
		d.SetString(dict["account"], 10)
		status := user.BlockchainUser.UiLoginVerify(d)
		fmt.Println("查询完毕，该账户身份为：", status)
		c.JSON(http.StatusOK, gin.H{
			"status": gin.H{
				"code":    http.StatusOK,
				"success": true,
...
func sendToSuperior(bts []byte) {
	if global.Cfg.Prev == nil {
		log.Println("该账户暂无添加上级...")
		return
	}
	fmt.Println("列车层内共识完成，向铁路局节点请求共识...")
	conn, err := net.Dial("tcp", "localhost:5"+global.Cfg.Prev[1])
	if err == nil {
		log.Println("send to superior:", conn.RemoteAddr())
		//fmt.Println("global.PortId:", global.PortId)
		g := Gossip{Command: CommandType_RequestConsensus, Message: bts}
		// proto marshal
		out, err := proto.Marshal(&g)
		if err != nil {
			panic(err)
...
func sendToSubordinates(bts []byte) {
	fmt.Println("send to subordinate")
	if global.Cfg.Next == nil {
		log.Println("该账户暂无添加下级...")
		return
	}
	chSend := make(chan struct{})
	for i := 0; i < len(global.Cfg.Next); i++ {
		go func(addr string) {
			conn, err := net.Dial("tcp", "localhost:5"+addr)
			if err == nil {
				log.Println("send to subordinate:", conn.RemoteAddr())
				//fmt.Println("global.PortId:", global.PortId)
				g := Gossip{Command: CommandType_AgreeConsensus, Message: bts}
				// proto marshal
				out, err := proto.Marshal(&g)
				if err != nil {
					panic(err)
